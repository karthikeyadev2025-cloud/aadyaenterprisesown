...
export default function ManagerPortal() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [leadsRes, staffRes, attRes, advRes, leaveRes] = await Promise.all([
      supabase.from('marketing_leads').select('*, assignee:app_users!assigned_to(full_name), executive:app_users!executive_user_id(full_name)').order('created_at', { ascending: false }).limit(10000),
  ...
  const requirementBreakdown = leads.reduce<Record<string, number>>((acc, l) => {
    acc[l.requirement] = (acc[l.requirement] || 0) + 1;
    return acc;
  }, {});

  const telecallers = staff.filter(s => s.role === 'telecaller');
  const executives = staff.filter(s => s.role === 'marketing_executive');
  ...
  const executivePerformance = executives.map(ex => ({
    ...ex,
