...
import {
  ...
  Camera, X, Navigation, Loader, Clock, LayoutDashboard,
  MessageSquare, RefreshCw, Calendar, AlertCircle, Search
} from 'lucide-react';
...
export default function ExecutivePortal() {
  ...
  const loadConversations = useCallback(async () => {
    if (!user) return;
    setConvLoading(true);
    const { data: myLeads } = await supabase.from('marketing_leads').select('id').eq('executive_user_id', user.id);
  ...
  async function handleAttendanceSelfie(blob: Blob, type: 'check_in' | 'check_out') {
    ...
    if (type === 'check_in') {
      ...
      await supabase.from('attendance_records').upsert(payload, { onConflict: 'staff_user_id,attendance_date' });
    } else {
      ...
      if (!fresh) { setAttCapturing(false); await loadAttendance(); return; }
      const checkInTime = fresh.check_in_time;
      let workHours = 0;
      if (checkInTime) {
        const inMs = new Date(checkInTime).getTime();
        workHours = Math.round(((now.getTime() - inMs) / 3600000) * 10) / 10;
      }
  ...
  async function uploadLeadPhoto(): Promise<string | null> {
    ...
    if (error) return null;
    const { data: { publicUrl } } = supabase.storage.from('lead-photos').getPublicUrl(data.path);
    return publicUrl;
  }
                      ...
                      <div className="p-4">
                        <div className="flex gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-amber-400/80 mt-1 font-medium">{lead.requirement}{lead.requirement ? ' · ' : ''}{new Date(lead.created_at).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}</p>
                            {lead.requirement_details && (
                              <p className="text-xs text-slate-400 mt-1 leading-relaxed line-clamp-2">{lead.requirement_details}</p>
                            )}
                            {lead.address && (
                              <p className="text-xs text-slate-500 mt-0.5 flex items-center gap-1"><MapPin className="w-2.5 h-2.5 shrink-0" />{lead.address}</p>
                            )}
                            {lead.location_address && lead.location_address !== lead.address && (
                              <p className="text-xs text-teal-500/80 mt-0.5 flex items-center gap-1"><MapPin className="w-2.5 h-2.5 shrink-0" />{lead.location_address}</p>
                            )}
                            {lead.latitude && lead.longitude && (
                              <a href={`https://www.google.com/maps?q=${lead.latitude},${lead.longitude}`} target="_blank" rel="noreferrer" className="text-teal-500 text-[10px] hover:underline flex items-center gap-0.5 mt-0.5">
