...
function HomeTab({ user, todayRecord, presentDays, leaveDays, lastPayslip, advances, leaves, attendance, onRefresh, setActiveTab }: {
  ...
  const recentlyApproved = [...leaves.filter(l => l.status === 'approved'), ...advances.filter(a => a.status === 'approved' || a.status === 'disbursed')].length;
