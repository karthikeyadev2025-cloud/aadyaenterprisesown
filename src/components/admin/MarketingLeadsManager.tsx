...
import { Users, Trash2, X, Download, Phone, MapPin, Search, MessageSquare, Calendar, RefreshCw, UserCheck, ChevronDown, Send, ChevronUp, Clock, FileText, AlertCircle, Tag, PhoneCall, Star, ArrowUpRight, CreditCard as Edit2, Save, BarChart2, Mail, Navigation, Building2, Pencil, Check } from 'lucide-react';
...
const STATUS_OPTIONS = ['new', 'called', 'interested', 'not_interested', 'not_answered', 'converted', 'closed', 'callback'];
...
const STATUS_LABELS: Record<string, string> = {
  ...
  converted: 'Converted', closed: 'Deal Closed', callback: 'Callback',
};
...
export default function MarketingLeadsManager() {
  ...
  const requirementOptions = useMemo(
    () => Array.from(new Set(leads.map(l => l.requirement).filter(Boolean))).sort(),
    [leads]
  );
  ...
  const toggleExpand = useCallback(async (lead: MarketingLead) => {
  ...
  }, [expandedId, remarksMap]);

  const sendRemark = useCallback(async (lead: MarketingLead) => {
    ...
    setTimeout(() => remarkEndRef.current?.scrollIntoView({ behavior: 'smooth' }), 100);
  }, [newRemark, remarkCallType]);

  function startEditRemark(r: LeadRemark) {
    setEditingRemarkId(r.id);
    setEditingRemarkText(r.remark);
  }

  async function saveEditRemark(leadId: string, remarkId: string) {
    if (!editingRemarkText.trim()) return;
    await supabase.from('lead_remarks').update({ remark: editingRemarkText.trim() }).eq('id', remarkId);
    setRemarksMap(prev => ({
      ...prev,
      [leadId]: (prev[leadId] || []).map(r => r.id === remarkId ? { ...r, remark: editingRemarkText.trim() } : r),
    }));
    setEditingRemarkId(null);
    setEditingRemarkText('');
  }

  async function deleteRemark(leadId: string, remarkId: string) {
    if (!confirm('Delete this remark permanently?')) return;
    await supabase.from('lead_remarks').delete().eq('id', remarkId);
    setRemarksMap(prev => ({
      ...prev,
      [leadId]: (prev[leadId] || []).filter(r => r.id !== remarkId),
    }));
    // Decrement follow_up_count
    setLeads(prev => prev.map(l => l.id === leadId ? { ...l, follow_up_count: Math.max(0, (l.follow_up_count || 1) - 1) } : l));
  }
  ...
  async function saveEdit(lead: MarketingLead) {
    setSaving(true);
    const isClosed = editForm.status === 'converted' || editForm.status === 'closed';
    const callbackIso = editForm.callback_date
      ? new Date(editForm.callback_date as string).toISOString()
      : null;
    await supabase.from('marketing_leads').update({
      status: editForm.status,
      remarks: editForm.remarks || null,
      callback_date: callbackIso,
      invoice_number: isClosed ? (editForm.invoice_number || null) : null,
      invoice_amount: isClosed ? (editForm.invoice_amount || null) : null,
      updated_at: new Date().toISOString(),
    }).eq('id', lead.id);
    setLeads(prev => prev.map(l => l.id === lead.id ? {
      ...l,
  ...
  const teamStats = useMemo(() => telecallers.map(tc => {
    const tcLeads = leads.filter(l => l.assigned_to === tc.id);
    const converted = tcLeads.filter(l => l.status === 'converted' || l.status === 'closed').length;
    const interested = tcLeads.filter(l => l.status === 'interested').length;
    const totalCalls = tcLeads.reduce((s, l) => s + (l.follow_up_count || 0), 0);
    const overdue = tcLeads.filter(l => l.callback_date && new Date(l.callback_date) < new Date() && l.status !== 'converted' && l.status !== 'closed').length;
  ...
  }).sort((a, b) => b.total - a.total), [telecallers, leads]);

  const pagedLeads = useMemo(() => filtered.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE), [filtered, page]);
              ...
              <div key={lead.id} className={`bg-slate-800/60 border rounded-xl transition-all ${isExpanded ? 'border-amber-500/40' : 'border-slate-700 hover:border-slate-600'}`}>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-2 mb-3">
                        ...
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium border ${PRIORITY_COLORS[lead.priority] || PRIORITY_COLORS.medium}`}>
                          {lead.priority === 'high' ? <><Star className="w-2.5 h-2.5 inline mr-0.5" />High</> : lead.priority === 'medium' ? 'Medium' : 'Low'}
                        </span>
                        {(lead.status === 'converted' || lead.status === 'closed') && lead.invoice_number && (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-1.5 mb-3">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-slate-500 text-xs w-16 shrink-0">Primary</span>
                          <a href={`tel:${lead.contact_number}`} className="text-white font-semibold text-sm hover:text-amber-400 transition-colors">{lead.contact_number}</a>
                        </div>
                        {lead.alternate_number && (
                          <div className="flex items-center gap-1.5">
                            <Phone className="w-3.5 h-3.5 text-slate-500 shrink-0" />
                            <span className="text-slate-500 text-xs w-16 shrink-0">Alternate</span>
                            <a href={`tel:${lead.alternate_number}`} className="text-slate-300 text-sm hover:text-amber-400 transition-colors">{lead.alternate_number}</a>
                          </div>
                        )}
                        {lead.email && (
                          <div className="flex items-center gap-1.5 md:col-span-2">
                            <Mail className="w-3.5 h-3.5 text-slate-500 shrink-0" />
                            <span className="text-slate-500 text-xs w-16 shrink-0">Email</span>
                            <a href={`mailto:${lead.email}`} className="text-slate-300 text-sm hover:text-amber-400 transition-colors truncate">{lead.email}</a>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="space-y-1.5 mb-3">
                        <div className="flex items-start gap-1.5">
                          ...
                          <span className="text-slate-500 text-xs w-16 shrink-0 pt-0.5">Area</span>
                          <span className="text-slate-200 text-sm">{lead.location || '—'}</span>
                        </div>
                        {lead.address && (
                          <div className="flex items-start gap-1.5">
                            <Building2 className="w-3.5 h-3.5 text-slate-500 shrink-0 mt-0.5" />
                            <span className="text-slate-500 text-xs w-16 shrink-0 pt-0.5">Address</span>
                            <span className="text-slate-300 text-sm leading-relaxed">{lead.address}</span>
                          </div>
                        )}
                        {lead.location_address && (
                          <div className="flex items-start gap-1.5">
                            <Navigation className="w-3.5 h-3.5 text-teal-500 shrink-0 mt-0.5" />
                            <span className="text-slate-500 text-xs w-16 shrink-0 pt-0.5">GPS</span>
                            <div className="min-w-0">
                              <span className="text-teal-300 text-sm leading-relaxed">{lead.location_address}</span>
                              {lead.latitude && lead.longitude && (
                                <a href={`https://www.google.com/maps?q=${lead.latitude},${lead.longitude}`} target="_blank" rel="noreferrer"
                                  className="block text-teal-500 text-[10px] hover:underline mt-0.5">
                                  Open in Google Maps ↗
                                </a>
                              )}
                            </div>
                        ...
                        {!lead.location_address && lead.latitude && lead.longitude && (
                          <div className="flex items-center gap-1.5">
                            <Navigation className="w-3.5 h-3.5 text-teal-500 shrink-0" />
                            <span className="text-slate-500 text-xs w-16 shrink-0">GPS</span>
                            <a href={`https://www.google.com/maps?q=${lead.latitude},${lead.longitude}`} target="_blank" rel="noreferrer"
                      ...
                      <div className="bg-amber-500/5 border border-amber-500/15 rounded-xl p-3 mb-3 space-y-2">
                        <div className="flex items-center gap-2">
                          <Tag className="w-3.5 h-3.5 text-amber-400 shrink-0" />
                          <span className="text-amber-300 text-xs font-semibold uppercase tracking-wider">Requirement</span>
                          <span className="text-white font-semibold text-sm">{lead.requirement || '—'}</span>
                        </div>
                        {lead.requirement_details && (
                          <div className="flex items-start gap-2 pl-5">
                            <FileText className="w-3 h-3 text-slate-500 shrink-0 mt-0.5" />
                            <p className="text-slate-300 text-sm leading-relaxed">{lead.requirement_details}</p>
                          </div>
                        )}
                      </div>
                      ...
                      {lead.lead_photo_url && (
                        <div className="mb-3">
                          <a href={lead.lead_photo_url} target="_blank" rel="noreferrer">
                            <img src={lead.lead_photo_url} alt="Lead photo" className="w-full max-h-40 object-cover rounded-xl border border-amber-500/20 hover:border-amber-500/50 transition-colors" />
                          </a>
                        </div>
                      )}

                      {/* ── Deal Closed / Converted block ── */}
                      {(lead.status === 'converted' || lead.status === 'closed') && (
                        <div className="bg-teal-500/10 border border-teal-500/25 rounded-xl p-3 mb-3 flex flex-wrap gap-6">
                          <div>
                            <p className="text-teal-400 text-[10px] font-semibold uppercase tracking-wider mb-0.5">Deal Status</p>
                            <p className="text-white font-bold text-sm">{STATUS_LABELS[lead.status]}</p>
                          </div>
                          {lead.invoice_number && (
                            <div>
                              <p className="text-teal-400 text-[10px] font-semibold uppercase tracking-wider mb-0.5">Invoice No.</p>
                              <p className="text-white font-bold text-sm">{lead.invoice_number}</p>
                            </div>
                          )}
                          {lead.invoice_amount && (
                            <div>
                              <p className="text-teal-400 text-[10px] font-semibold uppercase tracking-wider mb-0.5">Invoice Amount</p>
                              <p className="text-white font-bold text-sm">₹{Number(lead.invoice_amount).toLocaleString('en-IN')}</p>
                            </div>
                          )}
                          {!lead.invoice_number && (
                            <div className="flex items-center">
                              <span className="text-teal-500/70 text-xs italic">No invoice added yet — edit to add</span>
                            </div>
                          )}
                        </div>
                      )}
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500 mb-3">
                        <span>By: <span className="text-slate-400">{lead.collected_by}</span></span>
                        <span>Added: <span className="text-slate-400">{formatDateTime(lead.created_at)}</span></span>
                        {lead.last_called_at && <span>Last call: <span className="text-slate-400">{timeAgo(lead.last_called_at)}</span></span>}
                        ...
                        {lead.callback_date && (
                          <span className={isOverdue ? 'text-red-400' : 'text-orange-400'}>
                            <Clock className="w-3 h-3 inline mr-0.5" />Callback: {formatDateTime(lead.callback_date)}
                          </span>
                        )}
                        {(lead.status === 'converted' || lead.status === 'closed') && lead.invoice_number && (
                      ...
                      <div className="flex flex-wrap items-center gap-2 mb-3">
                        ...
                        {telecallerName && (
                          <span className="text-xs text-sky-400 flex items-center gap-1">
                            <Users className="w-3 h-3" />Assigned to {telecallerName}
                          </span>
                        )}
                      </div>
                ...
                {isEditing && (
                  <div className="border-t border-slate-700 p-4 bg-slate-800/40 space-y-3">
                    <p className="text-slate-300 text-sm font-semibold flex items-center gap-2"><Edit2 className="w-3.5 h-3.5 text-amber-400" /> Edit Lead Details</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      {/* Invoice — only for converted / closed */}
                      {(editForm.status === 'converted' || editForm.status === 'closed') && (
                        <>
                          <div>
                            <label className="block text-xs text-teal-400 mb-1 font-semibold">Invoice Number</label>
                            <input type="text" value={editForm.invoice_number || ''} onChange={e => setEditForm(f => ({ ...f, invoice_number: e.target.value }))}
                              placeholder="e.g. INV-001"
                              className="w-full px-3 py-2 bg-slate-900 border border-teal-500/40 rounded-xl text-white text-sm focus:outline-none focus:border-teal-500" />
                          </div>
                          <div>
                            <label className="block text-xs text-teal-400 mb-1 font-semibold">Invoice Amount (₹)</label>
                  ...
                  <div className="border-t border-slate-700 bg-slate-900/40">
                    ...
                    <div className="max-h-72 overflow-y-auto px-4 py-3 space-y-3">
                      {loadingRemarks === lead.id ? (
                        ...
                        const isEditingThis = editingRemarkId === r.id;
                        return (
                          <div key={r.id} className="relative pl-7">
                            ...
                            <div className={`bg-slate-800/60 border rounded-xl p-2.5 ${isEditingThis ? 'border-amber-500/40' : 'border-slate-700/40'}`}>
                              <div className="flex flex-wrap items-center gap-1.5 mb-1">
                                ...
                                <span className="text-slate-600 text-[10px]">{formatDateTime(r.created_at)}</span>
                                {/* Admin edit/delete controls */}
                                <div className="ml-auto flex items-center gap-1">
                                  {isEditingThis ? (
                                    <>
                                      <button onClick={() => saveEditRemark(lead.id, r.id)}
                                        className="p-1 rounded bg-amber-500/20 text-amber-400 hover:bg-amber-500/30 transition-colors" title="Save">
                                        <Check className="w-3 h-3" />
                                      </button>
                                      <button onClick={() => { setEditingRemarkId(null); setEditingRemarkText(''); }}
                                        className="p-1 rounded bg-slate-700 text-slate-400 hover:text-white transition-colors" title="Cancel">
                                        <X className="w-3 h-3" />
                                      </button>
                                    </>
                                  ) : (
                                    <>
                                      <button onClick={() => startEditRemark(r)}
                                        className="p-1 rounded bg-slate-700/60 text-slate-500 hover:text-amber-400 hover:bg-amber-500/10 transition-colors" title="Edit remark">
                                        <Pencil className="w-3 h-3" />
                                      </button>
                                      <button onClick={() => deleteRemark(lead.id, r.id)}
                                        className="p-1 rounded bg-slate-700/60 text-slate-500 hover:text-red-400 hover:bg-red-500/10 transition-colors" title="Delete remark">
                              ...
                              {isEditingThis ? (
                                <textarea
                                  ...
                                  onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); saveEditRemark(lead.id, r.id); } if (e.key === 'Escape') { setEditingRemarkId(null); setEditingRemarkText(''); } }}
                                  rows={2}
                                  autoFocus
                                  className="w-full px-2 py-1.5 bg-slate-900 border border-amber-500/40 rounded-lg text-white text-sm resize-none focus:outline-none"
                                />
                              ) : (
            ...
            <div className="flex items-center justify-between pt-2 pb-1">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  disabled={page === 0}
                  className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-xs disabled:opacity-40 transition-colors"
                >
                  Prev
                </button>
