...
import { Plus, CreditCard as Edit, Trash2, Save, X, Sun } from 'lucide-react';
...
export default function SolarManager() {
  ...
  const [editingBenefit, setEditingBenefit] = useState<Partial<SolarBenefit> | null>(null);
  const [editingType, setEditingType] = useState<Partial<SolarType> | null>(null);
  const [editingPractice, setEditingPractice] = useState<Partial<BestPractice> | null>(null);
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'benefits' && (
        <div className="space-y-4">
          <button
            onClick={() => setEditingBenefit({ icon: 'Sun', title: '', description: '', order_index: benefits.length })}
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-white">
                  {editingBenefit.id ? 'Edit Benefit' : 'New Benefit'}
                </h3>
      ...
      {activeTab === 'types' && (
        <div className="space-y-4">
          <button
            onClick={() => setEditingType({ icon: 'Home', title: '', size: '', features: [], order_index: solarTypes.length })}
            className="flex items-center space-x-2 px-4 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition-colors"
          >
            <Plus className="w-5 h-5" />
          ...
          {editingType && (
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-white">
                  {editingType.id ? 'Edit Solar Type' : 'New Solar Type'}
                </h3>
              ...
              <div className="space-y-4">
                ...
                <textarea
                  ...
                  onChange={(e) => setEditingType({ ...editingType, features: e.target.value.split('\n').filter(f => f.trim()) })}
                  rows={5}
                  className="w-full px-4 py-2 bg-slate-900 border border-slate-700 rounded-lg text-white"
                />
                <button
      ...
      {activeTab === 'practices' && (
        <div className="space-y-4">
          <button
            onClick={() => setEditingPractice({ practice: '', order_index: practices.length })}
            className="flex items-center space-x-2 px-4 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition-colors"
          >
            <Plus className="w-5 h-5" />
          ...
          {editingPractice && (
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-white">
                  {editingPractice.id ? 'Edit Practice' : 'New Practice'}
                </h3>
                <button onClick={() => setEditingPractice(null)}>
                  <X className="w-5 h-5 text-slate-400 hover:text-white" />
                </button>
              </div>
              <div className="space-y-4">
