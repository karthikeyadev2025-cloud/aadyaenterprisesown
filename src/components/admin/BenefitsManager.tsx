...
import { Plus, CreditCard as Edit, Trash2, Save, X, Award } from 'lucide-react';
...
export default function BenefitsManager() {
  ...
  const [editingBenefit, setEditingBenefit] = useState<Partial<CompanyBenefit> | null>(null);
  const [editingWhy, setEditingWhy] = useState<Partial<WhyChooseUs> | null>(null);
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'benefits' && (
        <div className="space-y-4">
          <button
            onClick={() => setEditingBenefit({ icon: 'Shield', title: '', description: '', gradient: 'from-blue-500 to-blue-700', order_index: benefits.length })}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setEditingBenefit({ ...editingBenefit, gradient: e.target.value })}
                    className="w-full px-4 py-2 bg-slate-900 border border-slate-700 rounded-lg text-white"
                  >
      ...
      {activeTab === 'why' && (
        <div className="space-y-4">
          <button
            onClick={() => setEditingWhy({ title: '', description: '', order_index: whyChoose.length })}
            className="flex items-center space-x-2 px-4 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition-colors"
          >
            <Plus className="w-5 h-5" />
            <span>Add Item</span>
          ...
          {editingWhy && (
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-white">
                  {editingWhy.id ? 'Edit Item' : 'New Item'}
                </h3>
                <button onClick={() => setEditingWhy(null)}>
                  <X className="w-5 h-5 text-slate-400 hover:text-white" />
                </button>
              </div>
              <div className="space-y-4">
