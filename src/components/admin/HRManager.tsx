...
import {
  ...
  CreditCard as Edit3, CheckCircle, AlertCircle, Phone, Mail, MapPin, Building2,
...
export default function HRManager() {
  ...
  const loadAll = useCallback(async () => {
    ...
    const [sRes, auRes, pRes, lRes, cRes, attRes, advRes] = await Promise.all([
      ...
      supabase.from('leave_requests').select('*, staff:staff_records(full_name, department)').order('created_at', { ascending: false }).limit(500),
  ...
  const refreshLeaves = useCallback(async () => {
    const { data } = await supabase.from('leave_requests').select('*, staff:staff_records(full_name, department)').order('created_at', { ascending: false }).limit(500);
    setLeaves((data as LeaveRequest[]) || []);
  }, []);

  const refreshAdvances = useCallback(async () => {
    const { data } = await supabase.from('salary_advance_requests').select('*, employee:app_users(full_name, role)').order('created_at', { ascending: false }).limit(200);
    setAdvances(data || []);
  }, []);

  const refreshPayroll = useCallback(async () => {
    const { data } = await supabase.from('payroll_records').select('*').order('year', { ascending: false }).order('month', { ascending: false });
    setPayroll(data || []);
  }, []);

  const refreshStaff = useCallback(async () => {
    const [sRes, auRes] = await Promise.all([
      supabase.from('staff_records').select('*').order('full_name'),
      supabase.from('app_users').select('id, full_name, role, email, phone, is_active, created_at, custom_role_id').order('full_name'),
    ]);
    setStaff(sRes.data || []);
  ...
  const refreshCRM = useCallback(async () => {
    const { data } = await supabase.from('office_crm_contacts').select('*').order('company_name');
    setCRM(data || []);
  }, []);
  ...
  const onLeaveCount = useMemo(() => staff.filter(s => s.status === 'on_leave').length, [staff]);
  const pendingLeaves = useMemo(() => leaves.filter(l => l.status === 'pending').length, [leaves]);
  const pendingAdvances = useMemo(() => advances.filter((a: any) => a.status === 'pending').length, [advances]);
  const thisMonth = new Date().getMonth() + 1;
  const thisYear = new Date().getFullYear();
  const pendingPayroll = useMemo(() => staff.filter(s => s.status === 'active' && !payroll.find(p => p.staff_id === s.id && p.month === thisMonth && p.year === thisYear)).length, [staff, payroll, thisMonth, thisYear]);
  const totalPayroll = useMemo(() => payroll.filter(p => p.month === thisMonth && p.year === thisYear && p.payment_status === 'paid').reduce((sum, p) => sum + p.net_pay, 0), [payroll, thisMonth, thisYear]);
      ...
      <div className="flex-1 overflow-auto p-4 md:p-6">
        {loading ? (
          ...
          <>
            ...
            {activeTab === 'staff' && <StaffTab staff={staff} appUsers={appUsers} onRefresh={refreshStaff} userId={user?.id} />}
            {activeTab === 'attendance' && <AttendanceViewTab attendance={attendance} onRefresh={loadAll} />}
            {activeTab === 'payroll' && <PayrollTab staff={staff} appUsers={appUsers} payroll={payroll} onRefresh={refreshPayroll} userId={user?.id} />}
            {activeTab === 'leave' && <LeaveTab leaves={leaves} staff={staff} onRefresh={refreshLeaves} userId={user?.id} />}
            {activeTab === 'advances' && <AdvancesTab advances={advances} onRefresh={refreshAdvances} userId={user?.id} />}
            {activeTab === 'crm' && <CRMTab contacts={crm} onRefresh={refreshCRM} userId={user?.id} />}
          </>
        )}
      </div>
    </div>
...
function HROverview({ staff, payroll, leaves, crm, activeCount, onLeaveCount, pendingLeaves, pendingPayroll, totalPayroll, onNavigate }: {
  ...
  const deptBreakdown = useMemo(() => staff.reduce<Record<string, number>>((acc, s) => {
...
function StaffTab({ staff, appUsers, onRefresh, userId }: { staff: StaffRecord[]; appUsers: any[]; onRefresh: () => void; userId?: string }) {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return !q ? appUsers : appUsers.filter(u =>
  ...
  const filteredStaff = useMemo(() => {
    ...
    return !q ? staff : staff.filter(s =>
      s.full_name.toLowerCase().includes(q) || s.designation.toLowerCase().includes(q) || s.department.toLowerCase().includes(q)
    );
  }, [staff, search]);
