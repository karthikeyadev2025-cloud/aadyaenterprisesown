...
export default function DashboardOverview() {
  ...
  async function loadStats() {
    ...
    let leadsQuery = supabase.from('marketing_leads').select('status, requirement, callback_date, assigned_to, created_at').limit(100000);
    ...
    const [leadsRes, usersRes, recentRes, allTimeTotalRes] = await Promise.all([
      leadsQuery,
    ...
    setStats({
      ...
      allTimeTotal: allTimeTotalRes.count ?? leads.length,
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: (dateFrom || dateTo) ? 'Leads (Filtered)' : 'Total Leads', value: (dateFrom || dateTo) ? stats.totalLeads : stats.allTimeTotal, icon: BarChart2, color: 'text-white', bg: 'bg-slate-700/60', iconColor: 'text-amber-400' },
