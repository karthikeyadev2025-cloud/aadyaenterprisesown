...
import { Plus, CreditCard as Edit, Trash2, Save, X, Camera } from 'lucide-react';
...
export default function CCTVManager() {
  ...
  const [editingPackage, setEditingPackage] = useState<Partial<CCTVPackage> | null>(null);
  const [editingBrand, setEditingBrand] = useState<Partial<CCTVBrand> | null>(null);
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'packages' && (
        <div className="space-y-4">
          <button
            onClick={() => setEditingPackage({ title: '', cameras: '', price: '', features: [], is_popular: false, order_index: packages.length })}
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-6">
              ...
              <div className="space-y-4">
                ...
                <input
                  ...
                  onChange={(e) => setEditingPackage({ ...editingPackage, cameras: e.target.value })}
                  className="w-full px-4 py-2 bg-slate-900 border border-slate-700 rounded-lg text-white"
                />
                <input
                  type="text"
      ...
      {activeTab === 'brands' && (
        <div className="space-y-4">
          <button
            onClick={() => setEditingBrand({ name: '', order_index: brands.length })}
            className="flex items-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
          >
            <Plus className="w-5 h-5" />
            <span>Add Brand</span>
